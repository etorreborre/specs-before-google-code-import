...
object patternMatchersSpec extends MatchersSpecification {
  "Pattern matchers" should { clearExample.before
    ...
    "[ok is syntactic sugar for true from the Sugar trait]" in {
      ...
      assertion(("a", "b", "c") aka "the triplet" must beLike {case ("a", _) => ok}) must 
    ...
    "provide a beNone matcher for options: List().find {_ == 2} must beNone" in {
      ...
      assertion(List(2).find {_ == 2} aka "searching 2 in a list:" must beNone[Int]) must 
    ...
    "provide a beAlsoNone matcher matching if 2 options are None at the same time" in {
      ...
      assertion(noneString aka "string presence" must beAlsoNone(Some("thing"))) must failWith("string presence 'Some(thing)' is not None")
