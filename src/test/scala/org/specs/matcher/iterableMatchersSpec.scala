...
object iterableMatchersSpec extends MatchersSpecification {
  "Iterable matchers" should { usingBefore { () => clearExample }
    "provide a 'must beEmpty' matcher on iterables: List() must beEmpty" in {
      ...
      assertion(List("1") aka "the list" must beEmpty) must failWith("the list List(1) is not empty")
    }
    "provide a 'must notBeEmpty' matcher on iterables: List(1) must notBeEmpty" in {
      ...
      assertion(List() aka "the list" must notBeEmpty) must failWith("the list List() is empty")
    }
    "provide a 'must contain' matcher on iterables: List(1) must contain(1) [alias: mustContain]" in {
      ...
      assertion(List("one", "two") aka "the list" must contain("three")) must failWith("the list 'List(one, two)' doesn't contain 'three'")
    }
    "provide a 'must notContain' matcher on iterables: List(1) must notContain(2) [alias: mustNotContain]" in {
      ...
      assertion(List("one", "two") aka "the list" must notContain("one")) must failWith("the list 'List(one, two)' contains 'one'")
    }
    "provide a 'must beIn' matcher on iterables: 'one' must beIn(List('one', 'two'))" in {
      ...
      assertion("three" aka "the element" must beIn(List("one", "two"))) must failWith("the element 'three' is not in 'List(one, two)'")
    }
    "provide a 'must notBeIn' matcher on iterables: 'three' must notBeIn(List('one', 'two'))" in {
      ...
      assertion("one" aka "the element" must notBeIn(List("one", "two"))) must failWith("the element 'one' is in 'List(one, two)'")
    }
    "provide a 'must exist' matcher on iterables: List('one', 'two') must exist {m: String => m.contains('w')} [alias: mustExist]" in {
      ...
      assertion(List("one", "two") aka "the list" must exist {m: String => m.contains("z")}) must failWith("no element verifies the property in the list 'List(one, two)'")
    }
    "provide a 'must notExist' matcher on iterables: List('one', 'two') must notExist {m: String => m.contains('z')}  [alias: mustNotExist]" in {
      ...
      assertion(List("one", "two") aka "the list" must notExist {m: String => m.contains("n")}) must failWith("at least one element verifies the property in the list 'List(one, two)'")
    }
    ...
    "List('one', 'two') must existMatch('[n-o]') [alias: mustExistMatch]" in {
      ...
      assertion(List("one", "two") aka "the list" must existMatch("[a-c]")) must failWith("no element matches '[a-c]' in the list 'List(one, two)'")
    }
    ...
    "List('one', 'two') must existMatch('[a-c]') [alias: mustNotExistMatch]" in {
      ...
      assertion(List("one", "two") aka "the list" must notExistMatch("[n-o]")) must failWith("at least one element matches '[n-o]' in the list 'List(one, two)'")
    }
    "provide a 'haveSize' matcher checking the size of a collection" in {
      ...
      assertion(List("one", "two") aka "the list" must haveSize(3)) must failWith("the list 'List(one, two)' doesn't have size 3")
    }
