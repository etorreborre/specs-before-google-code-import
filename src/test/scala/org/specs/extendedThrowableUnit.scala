...
object extendedThrowableUnit extends Specification with ExceptionSamples {
  ...
  "an extended Throwable with stack traces methods" ->- ex should provide {
    ...
    "a rethrowFrom method throwing this exception but removing stack trace elements having the caller class name" in {
      try { e.rethrowFrom(this) }
      catch {
...
trait ExceptionSamples extends Contexts {
  var e: Exception = _
  val ex = beforeContext { e = createException }
