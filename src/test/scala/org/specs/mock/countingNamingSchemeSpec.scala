...
object countingNamingSchemeSpec extends NamingSchemeContext {
  "A counting naming scheme".when[NSC] should {
    "use the CamelCase scheme for a single mock. 'JMocker' -> 'jMocker'" in { c: NSC =>
      c.scheme.defaultNameFor(classOf[JMocker]) must_== "jMocker"
    }
    "add a 2 to the second named mock. 'JMocker' -> 'jMocker 2'" in { c: NSC =>
