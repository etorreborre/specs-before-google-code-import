...
class Prop[T](val label: String,
  ...
  def executeThis = expected.map(exp => constraint.map(c => c.execute(exp)))
   ...
   */
  override def toHtml = {
    def statusClass = if (executed) status else "value"
    <td> {label} </td> ++ (
            if (executed)
 ...
 */
case object Prop {
  def apply[T](label: String, value: T, toCheck: => Any): Prop[T] = new Prop(label, Some(value), Some(AnyConstraint(() => toCheck)))
  def apply[T](label: String, value: T, f: (T, T) => Any): Prop[T] = new Prop(label, Some(value), Some(FunctionConstraint(f)))
  def apply[T](label: String, value: T, c: Constraint[T]): Prop[T] = new Prop(label, Some(value), Some(c))
}
...
case class MatcherProp[T](
    ...
    override var expected: Option[T], override val actual: Option[T], override val constraint: Option[MatcherConstraint[T]]) extends Prop(label, expected, actual, constraint) {
